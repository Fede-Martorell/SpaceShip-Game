...
public class Ufo extends MovingObjects{
    ...
    private Vector2D pathFollowing(){
        ...
        //calculamos la distancia hasta el modulo
    ...
    //calculamos la fuerza seek
    private Vector2D seekForce(Vector2D target){
        Vector2D desiredVelocty = target.subtract(getCenter());
        desiredVelocty = desiredVelocty.normalize().scale(maxVel);
        return desiredVelocty.subtract(velocity);
    }
    ...
    public void update() {
        ...
        if(!fireRate.isRunning()){
            ...
            //como trabajamos con angulos lo tenemos que normalizar.
            ...
            Laser laser = new Laser(getCenter().add(toPlayer.scale(width)),
                    toPlayer, Constants.LASER_VEL, currentAngle+ Math.PI/2,
                    Assets.redLaser,gameState);
        ...
        //le damos una pequeña rotacion
        angle += 0.05;
        //tiene colision con el resto de unidades, por eso llamos la funcion collideWith().
        collideWith();
        fireRate.update();
    }
    @Override
